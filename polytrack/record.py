...
insect_tracks = pd.DataFrame(columns = ['nframe', 'insect_num','x0','y0','area','species','confidence','status','model'])
...
if pt_cfg.POLYTRACK.SAVE_VIDEO_OUTPUT:
    ...
    out_video = cv2.VideoWriter(str(output_dir)+'video.avi', codec, fps, (width, height))

if pt_cfg.POLYTRACK.SAVE_TRACK_FRAME:
    codec = cv2.VideoWriter_fourcc(*pt_cfg.POLYTRACK.VIDEO_WRITER)
    out_track = cv2.VideoWriter(str(output_dir)+'track.avi', codec, fps, (width, height))
...
def verify_insects(for_predictions):
    ...
    for prediction in for_predictions:
        ...
        on_flower = check_on_flower(prediction)


        if (total_records >= pt_cfg.POLYTRACK.INSECT_VERIFICATION_MIN_FRAMES) and (bs_ratio < pt_cfg.POLYTRACK.INSECT_VERIFICATION_MIN_BS) and not on_flower:
...
def get_dark_spot(_prediction):
    ...
    if output_video: cv2.circle(track_frame, (int(x0), int(y0)), int(pt_cfg.POLYTRACK.DL_DARK_SPOTS_RADIUS), (255,255,255), -1)
...
def process_output_video(frame, track_frame,details_frame, _nframe, idle):
    ...
    if pt_cfg.POLYTRACK.SAVE_VIDEO_OUTPUT and not idle:
...
def prepare_output_video(frame, track_frame, details_frame, _nframe):
    ...
    details_frame = cv2.putText(details_frame, 'Frame: ' +str(_nframe), (20, 20), cv2.FONT_HERSHEY_DUPLEX , 0.8, (244, 230, 0), 1, cv2.LINE_AA)
...
def complete_tracking(_predicted_position):
    ...
    if pt_cfg.POLYTRACK.SIGHTING_TIMES: insects_sightings.to_csv(str(output_dir)+'sight_times.csv', sep=',')


def get_data_predictions(_nframe):
    
    _for_predictions = np.empty([0,5])
    _active_insects = insect_tracks.loc[(insect_tracks['nframe'] == _nframe)&(insect_tracks['status'] == 'In')]['insect_num'].values.tolist()
    _missing_insects = insect_tracks.loc[(insect_tracks['nframe'] == _nframe)&(insect_tracks['status'] == 'missing')]['insect_num'].values.tolist()
    ...
    for insect in _active_insects:
        ...
        if output_video: cv2.line(track_frame,(_x1,_y1),(_x0,_y0),track_colour(insect),2)
...
def record_BS_detections(_nframe, details_frame, _associated_det_BS):
    ...
    for record in _associated_det_BS:
        ...
        insect_record_BS = [_nframe, _insect_num, _x, _y, _area, _species, _confidence, _status, _model]
...
def record_DL_detections(_nframe, details_frame, _associated_det_DL):
    ...
    for record in _associated_det_DL:
        ...
        insect_record_DL = [_nframe, _insect_num, _x, _y, _area, _species, _confidence, _status, _model]
...
def record_missing(_nframe, _missing):
    ...
    for record in _missing:
        ...
        insect_record_missing = [_nframe, _insect_num, _x, _y, _area, _species, _confidence, _status, _model]
...
def record_new_insect(_frame,_nframe, _new_insect):
    ...
    for record in _new_insect:
        ...
        if check_sight_coordinates(record):
            ...
            insect_record_new = [_nframe, _insect_num, _x, _y, _area, _species, _confidence, _status, _model]
            insect_tracks.loc[len(insect_tracks)] = insect_record_new

            if pt_cfg.POLYTRACK.SIGHTING_TIMES:
                try:
                    time = cal_abs_time(_nframe, pt_cfg.POLYTRACK.VIDEO_START_TIME)
                except:
                    time = 0

                new_insect = [_species, _insect_num, time, np.nan]
...
def manual_verification(_frame,_insect_num, _x, _y,_species,_confidence):
    ...
    cv2.imwrite(str(output_dir)+str(_filename), _insect_image)
...
def evaluate_missing(_nframe, _insect_num):
    ...
    if ((_last_edge_det==True)&(_missing_frames>max_occlusions_edge)):
        _status ='out'
        save_track(_insect_num)
        
    elif(_missing_frames>max_occlusions):
        _status ='out'
        save_track(_insect_num)
...
def save_track(_insect_num):
    ...
    if pt_cfg.POLYTRACK.FILTER_TRACKS:
        ...
        if cal_cum_distance(last_detections) >= pt_cfg.POLYTRACK.FILTER_TRACKS_DIST_THRESHOLD:
            ...
            _insect_track.to_csv(str(output_dir)+str(_insectname)+'.csv', sep=',') #Save the csv file with insect track
    ...
    if pt_cfg.POLYTRACK.SIGHTING_TIMES:
        exit_time = cal_abs_time(_insect_track['nframe'][_insect_track.loc[_insect_track['insect_num'] == _insect_num, 'x0'].last_valid_index()], pt_cfg.POLYTRACK.VIDEO_START_TIME)
...
def plot_path(_insect_track, _insectname):
    ...
    plt.scatter(_insect_track['x0'], (height-_insect_track['y0']), c=_insect_track['nframe'],  cmap='RdYlBu_r', label = 'Recorded Path')
    plt.plot(_insect_track['x0'], (height-_insect_track['y0']))
    ...
    plt.tight_layout()    
    plt.savefig(str(output_dir)+str(_insectname)+'.png', format='png', dpi=100)
    #plt.show()
